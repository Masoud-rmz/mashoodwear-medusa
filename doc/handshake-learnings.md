...
### ۱۴۰۵-۰۶-۳۱ — عکس محصول در ویترین «No image»

- **علامت:** عکس در Medusa Admin هست؛ کارت ویترین placeholder نشان می‌دهد.
- **علت:** فایل محلی مدوسا اغلب URL نسبی `/static/...` است. ویترین روی ۵۱۷۳ است و فقط `/uploads` را به CMS پروکسی می‌کند، پس مرورگر فایل را از پورت اشتباه می‌خواهد. فرمت HEIC آیفون جداگانه در مرورگر باز نمی‌شود.
- **رفع:** `resolveMedusaAssetUrl` آدرس `/...` را با `VITE_MEDUSA_BACKEND_URL` کامل می‌کند (کارت، گالری، سبد، کاور کالکشن).
- **قابل‌انتقال:** در UI بعدی همین پیشوند را بگذار، یا در مدوسا `backend_url` را مطلق کن. JPG/PNG/WEBP کافی است.
